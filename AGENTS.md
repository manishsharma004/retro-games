...
### Non-obvious gotchas
- Keyboard input is delivered to the emulator via global `document` listeners (Nostalgist `respondToGlobalEvents`), so focus does not matter. However, synthetic/automated key events (computer-use) generally do NOT reach the WASM core, so keyboard gameplay can't be verified through automation — test keyboard input manually in a real browser.
